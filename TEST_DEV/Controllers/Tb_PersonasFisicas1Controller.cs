...
namespace TEST_DEV.Controllers
{
    public class Tb_PersonasFisicas1Controller : ApiController
    {
        ...
        // POST: api/Tb_PersonasFisicas1
        [ResponseType(typeof(Tb_PersonasFisicas))]
        public IHttpActionResult PostTb_PersonasFisicas(Tb_PersonasFisicas tb_PersonasFisicas)
        {
            //Evalua reglas de data annotations (NOmbre y APaterno requerido)
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            }
        ...
        // DELETE: api/Tb_PersonasFisicas1/5
        [ResponseType(typeof(Tb_PersonasFisicas))]
